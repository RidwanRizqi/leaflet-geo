...
public class RefKecamatanRepository {
    ...
    public List<RefKecamatan> findAll() {
        String sql = "SELECT KD_PROPINSI, KD_DATI2, KD_KECAMATAN, NM_KECAMATAN FROM PBB.REF_KECAMATAN ORDER BY KD_PROPINSI, KD_DATI2, KD_KECAMATAN";
        return oracleJdbcTemplate.query(sql, ROW_MAPPER);
    }

    public List<RefKecamatan> findByKdPropinsi(String kdPropinsi) {
        String sql = "SELECT KD_PROPINSI, KD_DATI2, KD_KECAMATAN, NM_KECAMATAN FROM PBB.REF_KECAMATAN WHERE KD_PROPINSI = ? ORDER BY KD_DATI2, KD_KECAMATAN";
    ...
    public List<RefKecamatan> findByKdPropinsiAndKdDati2(String kdPropinsi, String kdDati2) {
        String sql = "SELECT KD_PROPINSI, KD_DATI2, KD_KECAMATAN, NM_KECAMATAN FROM PBB.REF_KECAMATAN WHERE KD_PROPINSI = ? AND KD_DATI2 = ? ORDER BY KD_KECAMATAN";
    ...
    public RefKecamatan findByPrimaryKey(String kdPropinsi, String kdDati2, String kdKecamatan) {
        String sql = "SELECT KD_PROPINSI, KD_DATI2, KD_KECAMATAN, NM_KECAMATAN FROM PBB.REF_KECAMATAN WHERE KD_PROPINSI = ? AND KD_DATI2 = ? AND KD_KECAMATAN = ?";
    ...
    public List<RefKecamatan> findByNmKecamatanContaining(String namaKecamatan) {
        String sql = "SELECT KD_PROPINSI, KD_DATI2, KD_KECAMATAN, NM_KECAMATAN FROM PBB.REF_KECAMATAN WHERE UPPER(NM_KECAMATAN) LIKE UPPER(?) ORDER BY KD_PROPINSI, KD_DATI2, KD_KECAMATAN";
        return oracleJdbcTemplate.query(sql, ROW_MAPPER, "%" + namaKecamatan + "%");
    }

    public long count() {
        String sql = "SELECT COUNT(*) FROM PBB.REF_KECAMATAN";
    ...
    public List<RefKecamatan> findAllWithPagination(int offset, int limit) {
        String sql = "SELECT * FROM (SELECT ROWNUM rn, KD_PROPINSI, KD_DATI2, KD_KECAMATAN, NM_KECAMATAN FROM PBB.REF_KECAMATAN ORDER BY KD_PROPINSI, KD_DATI2, KD_KECAMATAN) WHERE rn > ? AND rn <= ?";
